...
export const getMatch = async (link: string) => {
  ...
  const fetchedMatch = {
    ...
    link: "https://cmc2.sportskeeda.com/live-cricket-score/" + data.topic_slug + '/ajax?lang=en' as string,
    winner: winner,
    status: getStatus(data.match_status),
    score: data.score_strip.map((score: any) => score.score)
  }
  return fetchedMatch;
}
